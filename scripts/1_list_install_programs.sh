...
function 1_installComposer() {
    ...
    *)
    ...
    if ! command -v composer &>/dev/null; then
        ...
        "$local_temp_curl_path" -sS https://getcomposer.org/installer | php -- --install-dir=/usr/local/bin --filename=composer
        if [ $? -eq 0 ]; then
...
1_installOpenSSH() {
    ...
    versions=$("$local_temp_curl_path" -s "$URL_OpenSSH" | grep -oP 'openssh-\d+\.\d+p?\d*\.tar\.gz' | sed -r 's/openssh-(\d+\.\d+p?\d*)\.tar\.gz/\1/' | sort -Vr | uniq)
